...
OBSERVED_RATE_HELP = (
    "观测率 = 红品数量 ÷ 生产次数，按当前材料范围和技能等级汇总。"
    "它描述目前样本中实际观察到的比例，不等于游戏的真实概率或官方概率。"
    "例如 5 / 90 = 5.556%；0% 只表示当前样本尚未出现红品，不证明真实概率为 0。"
)

WILSON_CI_HELP = (
    "根据红品数量和生产次数计算的 95% Wilson 得分区间。"
...
def render() -> None:
    ...
    show_chart(rate_with_ci_chart(levels, "level", "技能等级红品率"), "skill-level-ci")
