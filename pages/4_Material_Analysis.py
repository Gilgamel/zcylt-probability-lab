...
def render() -> None:
    ...
    metrics[1].metric("红品数量", overall.successes)
    metrics[2].metric("观测红品率", _format_rate(overall.observed_rate))
    ...
    show_chart(cumulative_rate_chart(cumulative, "官匠营累计红品率"), "material-cumulative")
    if not cumulative.empty:
        daily_display = cumulative[["date", "attempts", "red", "daily_rate", "rate"]].copy()
        ...
        st.dataframe(
            daily_display.rename(columns={
                "date": "日期", "attempts": "尝试次数", "red": "红品数量",
                "daily_rate": "当日红品率", "rate": "累计红品率",
            }),
            hide_index=True,
            width="stretch",
        )
    ...
    show_chart(rate_with_ci_chart(levels, "level", "技能等级红品率与 95% Wilson CI"), "material-level-ci")
