...
观测只保存日历日期。默认日期和仪表盘“今天”统一按
`America/Toronto` 计算，可通过非秘密环境变量 `APP_TIMEZONE` 调整。
...
观测结果按分类定义：官匠营只记录 `red_count`；马厩和灵禽院记录绿、蓝、紫、橙及
其他/未说明。官匠营未采集或不适用的字段保存为 `NULL`；马厩和灵禽院中未出现的
品质是明确的 0；仅官匠营的 NULL 与明确观测到的 0 区分。
数据管理页面以 `—` 显示 NULL。
